...
Let's go over some essentials to create amazing visualizations in GGPlot. We will use the `gapminder` dataframe. First, we'll load `tidyverse`, which is where `ggplot2` is store, and the `gapminder` dataframe.
```{r}
library(tidyverse)
library(gapminder)
